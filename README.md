...
Steam 版 Tangy TD（v1.0.393）简体 + 繁体中文补丁的单文件安装/卸载工具。
内置简体中文（台湾用语繁体）两套译文，安装后默认简体，可在游戏内
设置 → 语言 切换「繁體中文」。补丁数据（11 个补丁文件 + 11 个官方原版文件）
以 deflate 压缩内嵌在 exe 中，无需安装流程，游戏目录自动探测
（本程序所在目录 → 注册表 Steam 库 → 盘符简单搜索）。
...
- 两种中文占用游戏自带语言槽：简体中文（德语槽）、繁體中文（西班牙语槽，
  原官方西班牙语被替代）；在游戏内语言菜单切换
